...
def visualize_predictions(predictions):
    ...
    predictions_collect = predictions.select("Genre", "JP_Sales", "EU_Sales", "NA_Sales").collect()

    # Création des dictionnaires pour stocker les ventes par genre pour chaque région
    sales_by_genre = {genre: {"JP_Sales": 0, "EU_Sales": 0, "NA_Sales": 0} for genre in
    ...
    plt.bar(index, jp_sales, color='red', width=bar_width, label='JP_Sales')
    plt.bar(index, eu_sales, color='blue', width=bar_width, label='EU_Sales', bottom=jp_sales)
    plt.bar(index, na_sales, color='green', width=bar_width, label='NA_Sales',
            bottom=[jp + eu for jp, eu in zip(jp_sales, eu_sales)])

    plt.xlabel('Genre')
    plt.ylabel('Estimated Sales')
    plt.title('Estimated Sales by Genre and Region')
